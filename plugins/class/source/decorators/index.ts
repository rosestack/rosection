...
const createDecorator = <Callback extends DecoratorCallback>(name: string, callback: Callback) => {
  return (...args: Parameters<Callback>) => {
    return (target: any, propertyKey?: any, descriptor?: any) => {
      ...
      const isConstructor = target instanceof Function;
  ...
  return {
    ...
    onResolve(context) {
      ...
      const resolveProperty = () => {
        ...
        if (optional) {
          instance = context.rosection.resolveOr(token, undefined);
        } else {
          instance = context.rosection.resolve(token);
        }
...
const Cacheable = createDecorator("cacheable", (cacheArgs?: boolean) => {
  return {
    on: [
...
const Debounce = createDecorator("debounce", (wait: number) => {
  return {
    on: [
      DecoratorType.Method,
      DecoratorType.StaticMethod,
